form.addEventListener("submit", event => {
    ...
    if (!data.title) {
        hasError = true;
        isInvalid("title").insertAdjacentElement('afterend', errorMessage("title", window.APP_MESSAGES['quiz_form_validation.title.required']));
    }
    if (!data.description) {
        hasError = true;
        isInvalid("description").insertAdjacentElement('afterend', errorMessage("description", window.APP_MESSAGES['quiz_form_validation.description.required']));
    }
    if (!data.questionTimeLimit) {
        hasError = true;
        isInvalid("questionTimeLimit").insertAdjacentElement('afterend', errorMessage("questionTimeLimit", window.APP_MESSAGES['quiz_form_validation.time.required']));
    }
    ...
    questionBlocks.forEach((block, index) => {
        ...
        if (questionInput && !questionInput.readOnly && !questionInput.value.trim()) {
            ...
            questionInput.parentNode.insertBefore(
                errorMessage(`questions[${index}].question`, window.APP_MESSAGES['quiz_form_validation.question.required'], `invalid-questionId-${index}`),
                    ...
                    if (optionDiv) {
                        optionDiv.parentNode.insertBefore(
                            errorMessage(
                                ...
                                window.APP_MESSAGES['quiz_form_validation.option.required'],
